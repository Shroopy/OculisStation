...
/datum/controller/subsystem/processing/quirks/proc/AssignQuirks(mob/living/user, quirk_transfer = FALSE, client/applied_client, list/blacklist = list(), add_unique = TRUE) // OCULIS EDIT ADDITION: add blacklist arg, quirk_transfer, add_unique
	var/badquirk = FALSE
	for(var/quirk_name in applied_client.prefs.all_quirks)
		...
		if(ispath(quirk_type))
			...
			if(quirk_type in blacklist)
				continue
			// OCULIS EDIT ADDITION END
			if(user.add_quirk(quirk_type, quirk_transfer, override_client = applied_client, add_unique = add_unique, announce = FALSE)) // OCULIS EDIT add quirk_transfer, add_unique
